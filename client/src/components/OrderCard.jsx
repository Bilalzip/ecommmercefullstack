...
const OrderCard = () => {
  ...
  useEffect(() => {
    const findProducts = async (data) => {
      const response = await axios.post(`https://ecomninja.onrender.com/api/v1/order`, { data });
  ...
  const onSubmitStatus = async (id) => {
    ...
    const response = await axios.post("https://ecomninja.onrender.com/api/v1/order/status", { orderStatus, id });
    console.log(response);
  };

  const EditOrderDetails = async (id) => {
    const response = await axios.post("https://ecomninja.onrender.com/api/v1/order/remove", { id });
      ...
      <section>
        {orders?.map((item, index) => (
          item && item.productsDetails && item.productsDetails.length > 0 ? (
            <div key={index || item.productsDetails[index]?.slug} className='p-4 w-full md:h-96 h-1/4 flex md:flex-row font-sans flex-col md:justify-between md:m-8 mt-8'>
              <Link to={`/category/:category/${item.productsDetails[index]?.slug}`}>
                <img className='md:w-auto h-full rounded-md w-full' src={item.productsDetails[index]?.photo} alt="villa image" />
              </Link>
              <div className='flex flex-col md:ml-2 mt-3 md:mt-0'>
                <h1 className='text-3xl font-semibold'>{item.productsDetails[index]?.productName}</h1>
                {!isAdmin ? (
                  <div>
                    <span>Order Status : </span> {item.orderStatus}
                  </div>
                ) : (
                  <div className='flex flex-col md:flex-row mt-4 rounded-lg gap-4'>
                    <select
          ...
          ) : (<div key={index}>You have not placed any order yet</div>)
        ))}
      </section>
    </div>
  );
